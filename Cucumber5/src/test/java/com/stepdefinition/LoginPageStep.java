...
//	}
	@When("User should Search the Hotels {string} , {string} , {string}")
	public void user_should_Search_the_Hotels(String location, String hotels, String RoomType, io.cucumber.datatable.DataTable dataTable) {
		searchpage = new SearchPage();
		type(searchpage.getDropdown(), location);
		type(searchpage.getDropdown1(), hotels);
		type(searchpage.getDropdown2(), RoomType);
		List<String> noofRooms = dataTable.asList();
		String s = noofRooms.get(3);
		type(searchpage.getDropdown3(),s);
		
		List<String> adultperRooms = dataTable.asList();
		String s1 = adultperRooms.get(2);
		type(searchpage.getDropdown4(),s1);
		
		List<String> childrenperRoom = dataTable.asList();
		String s2 = childrenperRoom.get(2);
		type(searchpage.getDropdown5(),s2);
...
//	}
	...
	@When("User should enter {string} , {string} , {string} , {string} , {string} , {string} ,{string}")
	public void user_should_enter(String firstname, String lastname, String billingaddress, String creditcardno, String expirydateMonth, String expirydateyear, String cvvnumber, io.cucumber.datatable.DataTable dataTable) {
		bookpage = new BookPage();
		...
		List<String> cctype = dataTable.asList();
		String s = cctype.get(3);
		type(bookpage.getDropdown1(),s);
